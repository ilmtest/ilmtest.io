import { IconBrandGithub, IconBrandInstagram, IconHome } from '@tabler/icons-react';
...
const DrawOutlineButton = ({ href, label }) => {
    return (
        <a
            className="px-5 py-2 group relative px-4 py-2 font-medium text-slate-100 transition-colors duration-[400ms] hover:text-indigo-300"
            href={href}
        >
            <span>{label}</span>
            ...
            <div className="mx-auto w-full max-w-6xl px-4 py-12 sm:px-6 lg:px-8">
                <nav className="flex flex-wrap justify-center">
                    {navigation.main.map((item) => (
                        <DrawOutlineButton href="/hello" key={item.name} label={item.name} />
                    ))}
                </nav>
